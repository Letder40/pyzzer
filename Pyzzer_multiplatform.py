...
def get_winroute():
    pyzzer_file = __file__
    file_deleter = re.compile("[A-Za-z0-9_\-]*\.py")
    pyzzer_folder = file_deleter.sub("", pyzzer_file)
    quizes_folder = pyzzer_folder + "quizes"
    return quizes_folder
...
def ls():
    if sistema == "Linux":
        ...
        command = 'dir /b "{quizes_folder}"'.format(quizes_folder=quizes_folder)
        os.system(command)
...
def create():
    ...
    while(error):
        ...
        if sistema == "Windows":
            quizes_folder = get_winroute()
            if os.path.exists(quizes_folder + "/" + file_name):
                error = True
    ...
    for i in range(numero_preguntas):
        ...
        if i+1 == numero_preguntas:
            ...
            if save == "yes" or save == "y" or save == "YES":
                ...
                if sistema == "Windows":
                    quizes_folder = get_winroute()
                    f = open(quizes_folder + "/" + file_name , "a")
                # Linux
                else:    
...
def use():
    ...
    if sistema == "Windows":
        file = open(quizes_folder + "/" + file_name, "r")
    else:            
